    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <a href="<?=base_url('kunjungan/add')?>"><button class="btn-primary btn btn-md">Tambah</button></a>
                    <br><br>
...
<script>
    $(document).ready(function(){
       $('#data_list').DataTable( {
            ...
            "serverSide": true,
            "ajax": {
                "url": "<?=base_url('kunjungan/kunjungan_json'.$jenis.$sub)?>",
                "type": "POST"
            },
            "aaSorting": [[ 1, "desc" ]],
            "columns": [
                    ...
                    $.ajax({
                        ...
                        success : function(data){
                            $.each(data,function(i,v){
                                daftarHadir+="<a href='<?php echo base_url()."kunjungan/word_absensi/"?>"+row.id+"?id_tipe="+v.id_tipe+"' class='dropdown-item'>"+v.tipe+"</a>";
                            })
                            setTipePegawai(daftarHadir)
                        }
                    })
                    function setTipePegawai(paramDaftarHadir){
                        daftarHadir = paramDaftarHadir 
                    }


                    return '<a '+(row.is_edit != 1 ? 'hidden' : '')+' href="<?=base_url('kunjungan/tinjau')?>/'+row.id+'" class="btn btn-sm btn-info text-white">Tinjau</a>&nbsp;<a href="<?=base_url('kunjungan/delete')?>/'+row.id+'" class="btn text-white btn-sm btn-danger" onclick="return confirm(\'Are you sure you want to delete this item?\');">Hapus</a>&nbsp;<div class="dropdown"><button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown">Daftar Hadir</button><div class="dropdown-menu">'+daftarHadir+'</div></div>&nbsp;<a href="<?=base_url('kunjungan/word')?>/'+row.id+'/false" class="btn btn-sm btn-primary text-white" target="_blank"><span class="fa fa-eye"></span> Laporan</a>&nbsp;<a href="<?=base_url('kunjungan/word')?>/'+row.id+'" class="btn btn-sm btn-primary text-white" target="_blank"><span class="fa fa-download"></span> Laporan</a>&nbsp;'+(row.jenis_kunjungan != 1  ? '<a href="<?=base_url('kunjungan/word_hearing')?>/'+row.id+'" class="btn btn-sm btn-primary text-white" target="_blank">Laporan Hearing</a>' : '');
                }}
            ],
        } );
    });
    ...
    function getAbsensi(eq){
        ...
        $.ajax({
            type: "get",
            url: '<?=base_url('kunjungan/get_absensi')?>'+'/'+id, //json get site
            dataType : 'json',
