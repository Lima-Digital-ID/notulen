...
<script>
    $(document).ready(function(){
       $('#data_list').DataTable( {
            ...
            "serverSide": true,
            "ajax": {
                "url": "<?=base_url('kunjungan/kunjungan_json')?>",
                "type": "POST"
            },
            "aaSorting": [[ 1, "desc" ]],
            "columns": [
                ...
                {"data": "tipe_kunjungan", "render": function(data, type, row){return tipeKunjungan(row.tipe_kunjungan)}, "class" : 'text-center'},
                {"data": "sub_tipe", "class" : 'text-center'},
                {"data": "id", "class" : 'text-center', "render": function(data, type, row){
                    return '<a '+(row.is_edit != 1 ? 'hidden' : '')+' href="<?=base_url('kunjungan/tinjau')?>/'+row.id+'" class="btn btn-sm btn-info text-white">Tinjau</a>&nbsp;<a href="<?=base_url('kunjungan/delete')?>/'+row.id+'" class="btn text-white btn-sm btn-danger" onclick="return confirm(\'Are you sure you want to delete this item?\');">Hapus</a>&nbsp;<button type="button" class="btn btn-primary btn-sm" data-target="#modalAbsensi" data-id="'+row.id+'" onclick="getAbsensi(this)" data-toggle="modal">Daftar Hadir</button>&nbsp;<a href="<?=base_url('kunjungan/word')?>/'+row.id+'" class="btn btn-sm btn-primary text-white" target="_blank">Laporan</a>&nbsp;'+(row.tipe_kunjungan == 1  ? '<a href="<?=base_url('kunjungan/word_hearing')?>/'+row.id+'" class="btn btn-sm btn-primary text-white" target="_blank">Laporan Hearing</a>' : '');
                }}
            ],
        } );
    });
...
<script type="text/javascript">
    $(document).ready(function() {
        var userDataTable = $('#userTable').DataTable({
            ...
            'ajax': {
                'url': '<?= base_url() ?>Kunjungan/kunjungantahun_json',
                'data': function(data) {
